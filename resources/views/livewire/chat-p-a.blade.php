<div>
    <div wire:poll.5s id="scroll" class="scroll-content">
        ...
        <div class="card d-inline-block mb-3 {{ Auth::user()->username == $c->id_pengirim ? 'float-right' : 'float-left' }} mr-2">
            ...
            <div class="card-body">
                <div class="d-flex flex-row pb-2"><a class="d-flex" href="#"><img alt="Profile Picture" @if(Auth::user()->username == $c->id_pengirim ) src="{{asset('image/fotomhs/')}}/{{$idrel->foto ?? 'dosen.png'}}" @else src="{{asset('image/fotomhs/')}}/{{'dosen.png'}}" @endif class="img-thumbnail border-0 rounded-circle mr-3 list-thumbnail align-self-center xsmall"></a>
                    <div class="d-flex flex-grow-1 min-width-zero">
                        <div class="m-2 pl-0 align-self-center d-flex flex-column flex-lg-row justify-content-between min-width-zero">
                            <div class="min-width-zero">
                                <p class="mb-0 truncate list-item-heading">{{ Auth::user()->username == $c->id_pengirim ? Auth::user()->name : $dosen->nama }}</p>
                            </div>
                        </div>
                    </div>
                </div>
