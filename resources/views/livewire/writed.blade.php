<div>
    <form wire:submit.prevent="store">
        <div class="chat-input-container d-flex justify-content-between align-items-center"><input class="form-control flex-grow-1" autocomplete="false" wire:model="message" name="message" type="text" {{$statusc == 2 ? 'disabled' : ''}} placeholder="{{$statusc == 2 ? 'Konsultasi Telah Berakhir, Buat Konsultasi Baru Untuk Memulai Percakapan' : 'Ketik Sesuatu...'}} ">
            <div><button type="button" wire:click="modalClean" data-toggle="modal" data-target="#exampleModal" class="btn btn-outline-primary icon-button large"><i class="simple-icon-paper-clip"></i></button>
                <button type="submit" class="btn btn-primary icon-button large"><i class="simple-icon-arrow-right"></i></button>
                @if($statusc == 2)
                @else
                <button data-toggle="modal" data-target="#chatseles" type="button" class="btn btn-danger icon-button large"><i class="simple-icon-close"></i></button>

                @endif
            </div>
        </div>
    </form>
    <div class="modal fade" wire:ignore.self id="chatseles" data-backdrop="static" tabindex="1" role="dialog" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-body">
                    <h6 class="mb-4">Mengakhiri Chat</h6>
                    <form wire:submit.prevent="saran">
                        <label for="">Saran / Masukan</label>
                        <textarea name="messagef" wire:model="messagef" class="form-control " cols="30" rows="5"></textarea>
                        <hr>
                        <div class="text-right">
                            <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button> <button type="submit" class="btn btn-primary"> Kirim </button>
                        </div>
                    </form>
                </div>
            </div>
